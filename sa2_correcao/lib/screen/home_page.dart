...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _loadPreferences() async {
    ...
    setState(() {
      ...
      _idioma = _prefs.getString('${email}_idioma') ??
          'pt-br'; // Obtém o estado atual do tema escuro ou define como falso se não houver valor
    });
  }
  ...
  Widget build(BuildContext context) {
    return AnimatedTheme(
      ...
      duration: const Duration(milliseconds: 500), // Define a duração da transição
      child: Scaffold(
        appBar: AppBar(
          title:
              const Text('Armazenamento Interno'), // Título da barra de aplicativos
        ),
        body: Center(
          child: Column(
            children: [
              Switch(
                value: _darkMode,
                // Valor do interruptor baseado no estado atual do tema escuro
                onChanged: (value) {
